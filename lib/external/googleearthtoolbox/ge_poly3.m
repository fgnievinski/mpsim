function output = ge_poly3(X,Y,Z,varargin)
% Reference page in help browser: 
% 
% <a href="matlab:web(fullfile(ge_root,'html','ge_poly3.html'),'-helpbrowser')">link</a> to html documentation
...
for x = 1:length(polyStartIx)
    ...
    if (polyEndIx(x)-polyStartIx(x))>=2
        ...
        if autoClose&&...
                (X(polyEndIx(x))~=X(polyStartIx(x))||...
                 Y(polyEndIx(x))~=Y(polyStartIx(x))||...
                 Z(polyEndIx(x))~=Z(polyStartIx(x)))
            if msgToScreen
                disp('Closing unclosed polygon...')
            end
            coordinates = [ coordinates, ...
                          sprintf('%.6f,%.6f,%.6f ',...
                          X(polyStartIx(x)), Y(polyStartIx(x)), Z(polyStartIx(x))),10];
            if msgToScreen
