...
function y = besselj0_fast_m (x)
    if ~isreal(x)
        error('MATLAB:besselj0_fast:nonReal', ...
            'Complex numbers not allowed as input.');
    end
    ...
    i = (0<x & x<=4);  
        y(i) = doit_le4 (x(i));
    i = (x>4);       
    %i = ~i;  % WRONG! (should NOT include x==0).
        y(i) = doit_gt4 (x(i));
end
