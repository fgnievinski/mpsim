function X = trisolve_sym (B, Q, p, uplow)
    ...
    if ~strncmpi(uplow, Q.uplow, 1)
        error ('packed:trisolve_sym:badUplow', ...
            '"uplow" asked is different than matrix''s uplow property.');
    end

    if ~strcmp(get_caller_name, 'trisolve'),
        error ('packed:trisolve_sym:privFunc', 'This is a private function.');
    end

    error (check_trisolve(B, Q));

    [info, X] = call_lapack ('sp', 'trs', ...
        Q.uplow, Q.order, size(B,2), Q.data, p, B, size(B,1));
    myassert (info >= 0);
end
...
%!error
%! s = lasterr ('', '');
%! trisolve_sym (packed(eye(2)));

%!test
%! % trisolve ()
%! s = lasterror;
%! myassert (s.identifier, 'packed:trisolve_sym:privFunc');
...
%! [R, p] = bk(A);
%!     myassert (ispacked(R));
%! [x, rcond] = trisolve (b, {R, p}, 'sym', norm(A, 1));
...
%! error = x - x_full;
%! 
%! %disp({precision, matrix_type, issorted(p), eps(precision), rmse(error), rcond});  % DEBUG
%! 
%! if rcond > 0.1
