...
sph_harm_degree, load_redundant, switch_left_right, load_extended)
    ...
    if ( isempty(comp.rhcp) || isempty(comp.lhcp) )
        if ~strcmpi(radome, 'none')
            radome2 = 'none';
            warning('snr:ant:noRad', ...
               'No antenna pattern data found for antenna "%s" with radome "%s"; trying radome "%s".', ...
               model, radome, radome2);
            comp = snr_setup_ant_comp (comp_type, model, radome2, freq_name, ...
                sph_harm_degree, load_redundant, switch_left_right, load_extended);
            return;
        end
        ...
        warning('snr:ant:noFreq', ...
           ...
           freq_name, freq_name2, model, radome);
        comp = snr_setup_ant_comp (comp_type, model, radome, freq_name2, ...
            sph_harm_degree, load_redundant, switch_left_right, load_extended);
        return;
    end
