function sett_bias = snr_bias_settings ()
    ...
    sett_bias.inv_ratio = false;  % [scalar] Normally the nominal reflection or direct phasors are multiplied by the respective bias phasor; divide, instead? (logical/Boolean)
    ...
    sett_bias.fit.trend_only = [];  % [scalar] Fit only trend (logical/Boolean)
    ...
    sett_bias.fit.height_domain = [];  % [vector or struct] Domain of heights to test in spectral analysis; see mplsqfourier_height.m and lsqfourier_period.m for format.
