function [coeff, snr_ratio_trend, snr_ratio_resid, inv_ratio] = snr_bias_fit_trend (...
snr_meas, indep, ...
snr_simul_trend, snr_simul_trend2, ...
degree, doprefit, dopostfit, plotit, inv_ratio)
    if (nargin < 2),  indep = [];  end
    if (nargin < 3),  snr_simul_trend = 1;  end
    if (nargin < 4),  snr_simul_trend2 = 1;  end
    if (nargin < 5),  degree = [];  end
    if (nargin < 6),  doprefit = [];  end
    if (nargin < 7),  dopostfit = [];  end
    if (nargin < 8),  plotit = [];  end
    if (nargin < 9),  inv_ratio = [];  end
    if isempty(inv_ratio),  inv_ratio = true;  end
    if isempty(dopostfit),  dopostfit = true;  end
    %dopostfit = false;
    % inv_ratio is preferable because SNR measurements might be 
    % near zero in deep fadings where their reciprocal would be near infinite;
    % dividing measurements (with fringes) by simulations (without fringes)
    % yields ratio values that deviate less in deep fades.
    
    if ~isscalar(snr_simul_trend),  snr_simul_trend(isnan(snr_meas)) = NaN;  end
    
    snr_ratio = snr_simul_trend ./ snr_meas;
    if inv_ratio,  snr_ratio = 1./snr_ratio;  end
      if plotit,  figure, hold on, plot(indep, snr_meas, 'b'), plot(indep, setel(snr_simul_trend, isnan(snr_meas), NaN), 'r');  end
      if plotit,  figure, plot(indep, snr_ratio, '-k');  end

    [coeff, snr_ratio_trend, snr_ratio_resid] = snr_bias_fit_trend_aux (...
        snr_ratio, indep, degree, doprefit, plotit);
      if plotit,  doplotit (indep, snr_meas, snr_simul_trend, snr_ratio, snr_ratio_trend, snr_ratio_resid);  end

    %dopostfit = false;  % DEBUG
    if ~dopostfit,  return;  end
    [coeff2, snr_ratio_trend2, snr_ratio_resid2] = snr_bias_fit_trend_aux2 (...
        snr_ratio, indep, ...
        snr_simul_trend, snr_simul_trend2, snr_ratio_trend, ...
        degree, doprefit, plotit);
...
%%
function [coeff, snr_fit, snr_resid] = snr_bias_fit_trend_aux (...
snr, indep, degree, doprefit, plotit)
    coeff = snr_bias_power_fit (snr, indep, degree, doprefit);
...
%%
function [coeff2, snr_ratio2_trend, snr_ratio_resid2] = snr_bias_fit_trend_aux2 (...
snr_ratio, indep, ...
snr_simul_trend, snr_simul_trend2, snr_ratio_trend, ...
degree, doprefit, plotit)
    ...
    [coeff2, snr_ratio2_trend, snr_ratio_resid2] = snr_bias_fit_trend_aux (...
        snr_ratio2, indep, degree, doprefit, plotit);
