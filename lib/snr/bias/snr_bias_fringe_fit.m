function [coeff, snr_meas_detrended, snr_meas_detrended_fit, pre] = snr_bias_fringe_fit (...
snr_meas, elev, ...
...
refine_peak, height_domain, wavelength, plotit, inv_ratio)
    ...
    if ~isequaln(pre.elev, elev)
        pre.elev = elev;
        pre.idx_nan = idx_nan;
        pre.J = [];
        pre.peak_simul = [];
        warning('snr:snr_bias_fringe_fit:badPre', ...
            'Inapplicable pre-processed results (J); ignoring it.')
    end
    if ~isequaln(pre.idx_nan, idx_nan)
        pre.idx_nan = idx_nan;
        pre.peak_simul = [];
        %warning('snr:snr_bias_fringe_fit:badPre', ...
    ...
    [peak_meas, snr_meas_detrended_fit, snr_meas_detrended, pre.J] = snr_bias_sinusoid_fit (...
    ...
    % TODO: snr_bias_chirp_fit -- chirp fitting (hint: lin freq fit; chirplet transform?).
      
    if isfieldempty(pre, 'peak_simul')
        [pre.peak_simul, snr_simul_detrended_fit] = snr_bias_sinusoid_fit (...
            snr_simul_detrended, elev, degree, refine_peak, ...
            height_domain, wavelength, pre.J, plotit);  %#ok<N%#ok<MSNU> ASGU>
    ...
    coeff = struct();
    coeff.power  = snr_bias_power_fit(peak_num.power/peak_den.power, 0, 0);
