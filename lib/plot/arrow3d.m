...
%Set default data
if nargin<3 || isempty(ang),  ang=30; end
if nargin<4 || isempty(ltype),  ltype='cylinder'; end
if nargin<5 || isempty(p),  p=[0.25,0.1]; end
if nargin<6 || isempty(n),  n=[20,10]; end
...
[m,n] = size(xdata);
if isempty(z)
    z=zeros(size(x));%ÔÚ¶Ô¶þÎ¬Êý¾�?½ø�?�?�?ý×ªÊ±£¬z²ÎÊýÊäÈë¿ÉÒÔÊÇ¿Õ¾ØÕó
end
