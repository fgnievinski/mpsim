function [h_line, h_band] = errorband2 (x, y, e, ...
line_style, band_color, band_transparency, single_point_dy, edge_color)
%ERRORBAND2  Filled-in error bands, vertical variant.
% 
%   See also: ERRORBAR
    ...
    if (nargin < 7) || isempty(single_point_dy),  single_point_dy = 0.5;  end
    ...
    if any(idx)
        h_band = errorband2_nan (idx, x, l, u, y, band_color, single_point_dy);
    else
        h_band = fill (...
            [y(:)-l(:); flipud(y(:)+u(:))], ...
            [x(:); flipud(x(:))], ...
...
%%
function h_band = errorband2_nan (idx, x, l, u, y, band_color, single_point_dy)
    hold on
    ind = find(idx);
    ind = [0; ind; numel(y)+1];
    ind4 = [];
